...
perimeter( #circle{radius=Radius} ) 							-> 2 * math:pi() * Radius;
perimeter( #rectangle{length=Length, width=Width} ) 			-> (2 * Length) + (2 * Width);
perimeter( #triangle{side1=Side1, side2=Side2, side3=Side3} ) 	-> Side1 + Side2 + Side3.


area( #circle{radius=Radius} ) 								-> math:pow(math:pi() * Radius, 2);
