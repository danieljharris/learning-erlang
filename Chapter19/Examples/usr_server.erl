...
-include("usr.hrl"). -define(TIMEOUT, 30000).
...
%% Exported Client Functions %% Operation & Maintenence API
start() -> start( get_path() ). %start("usrDb").
start(FileName) ->
...
add_usr(PhoneNum, CustId, Plan) when Plan==prepay; Plan==postpay -> call({add_usr, PhoneNum, CustId, Plan}).

delete_usr(CustId) -> call({delete_usr, CustId}).

set_service(CustId, Service, Flag) when Flag==true; Flag==false -> call({set_service, CustId, Service, Flag}).

set_status(CustId, Status) when Status==enabled; Status==disabled-> call({set_status, CustId, Status}).

delete_disabled() -> call(delete_disabled).
...
request({delete_usr, CustId}) -> usr_db:delete_usr(CustId);
...
request(delete_disabled) -> usr_db:delete_disabled().
